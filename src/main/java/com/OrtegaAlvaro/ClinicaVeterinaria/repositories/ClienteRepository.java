 ...
 * Gestiona las operaciones de base de datos relacionadas con los propietarios
 * de mascotas,
 * incluyendo búsquedas exactas por identificación legal y búsquedas aproximadas
 * por nombre.
 */
@Repository
public interface ClienteRepository extends JpaRepository<Cliente, Long> {
     ...
     * Utilizado para validaciones de unicidad y búsquedas directas en procesos
     * administrativos.
     *
     ...
     * La consulta contiene una cláusula 'LIKE' implícita y es insensible a
     * mayúsculas/minúsculas,
     * lo que la hace ideal para barras de búsqueda en tiempo real o filtrado de
     * listados.
     *
    ...
    @Query("SELECT c FROM Cliente c WHERE LOWER(c.nombre) LIKE LOWER(CONCAT('%', :search, '%')) OR LOWER(c.apellidos) LIKE LOWER(CONCAT('%', :search, '%')) OR LOWER(c.dni) LIKE LOWER(CONCAT('%', :search, '%'))")
